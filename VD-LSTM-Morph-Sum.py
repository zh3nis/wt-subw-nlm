...
def parse_args():
  ...
  parser.add_argument('--prefix', default='VD-LSTM-Morph-Sum',
...
def read_data(args, config):
  ...
  max_word_len = int(np.percentile(word_lens_in_morph, 100))
...
class Model:
  ...
  def __init__(self, config, word_ix_to_syl_ixs, need_reuse=False):
    ...
    for word_ix in range(word_vocab_size):
      words_in_morphs.append(word_ix_to_morph_ixs[word_ix])
